...
import { getPodcastByFeedUrl, getPodcastById, ingestPodcast } from '@/server/ingest/podcast';
...
export async function GET(request: NextRequest) {
  ...
  if (id) {
    ...
    if (isNaN(podcastId)) {
      return NextResponse.json({ message: 'parameter `id` must be a number' }, { status: 400 });
    }
  ...
  if (!url) {
    return NextResponse.json({ message: 'parameter `url` or `id` required' }, { status: 400 });
  }
